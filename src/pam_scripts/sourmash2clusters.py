...
def process_cluster(manifest, label, rows):
    with open(f"{label}.cluster.csv", "w") as f:
        for name, filename in manifest.loc[rows["name"], "filename"].items():
            print(name, filename, sep=",", file=f)
    paths = [
        f"sketches/{loc}" for loc in manifest.loc[rows["name"], "internal_location"]
    ]
    return (label, paths)
...
def main():
    ...
    embedding_clusters = embedding.groupby("label")
